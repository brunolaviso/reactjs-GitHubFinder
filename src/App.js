...
class App extends Component {
  ...
  getUser = (e) =>
  {
    ...
    axios.get(`${url}/${user}?client_id=${client_id}&client_secret=${client_secret}`).then(({data}) => this.setState({ user: data }));
    axios.get(`${url}/${user}/repos?per_page=${count}&sort=${sort}&client_id${client_id}&client_secret${client_secret}`).then(({data}) => this.setState({ repos: data }));
  }
